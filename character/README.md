...
1. **An E-commerce Product Catalog:** Querying for a specific product by ID `(character(id:...)`) or browsing paginated lists of products in a category `(characters(page:...)` or `episodes(page:...)`).
2. **A Blog or News Platform:** Fetching a specific article or a list of articles with pagination. The Episode type is analogous to a blog post.
3. **A Social Media Feed:** Loading paginated posts, comments, or a list of users.
